...
referralBtn.setAttribute('aria-label', 'Share referral link');
const taskNotification = document.createElement('div');
taskNotification.id = 'task-notification';
taskNotification.className = 'notification';
...
function initWelcomeButton() {
  ...
  welcomeBtn.addEventListener('click', () => {
    gsap.to(welcomePage, { opacity: 0, duration: 0.5, onComplete: () => {
...
// Додавання реферального посилання та завдань
document.getElementById('profile')?.appendChild(referralBtn);
document.getElementById('profile')?.appendChild(taskNotification);
  ...
  const animateBall = () => {
    ...
    if (y < plinkoCanvas.height - 50) {
      ...
      if (multiplier > 1) {
        ...
        const audio = new Audio('https://example.com/win-sound.mp3');
...
function updateMatches() {
  ...
  mockMatches.forEach(match => {
    ...
    matchCard.innerHTML = `
      ...
      <button class="btn-bet" data-i18n="place_bet" aria-label="Place bet">Зробити ставку</button>
      <button class="btn-quick-bet" data-i18n="quick_bet" aria-label="Quick bet">Швидка ставка</button>
      <button class="btn-stream" data-video="${match.video}" data-subtitles="${match.subtitles}" data-i18n="live_stream" aria-label="Watch live stream">Пряма трансляція</button>
...
chatToggleBtn?.addEventListener('click', () => {
  ...
  gsap.to(chatSupport, { x: chatSupport.classList.contains('open') ? 0 : 300, duration: 0.3 });
});

chatCloseBtn?.addEventListener('click', () => {
  chatSupport.classList.remove('open');
  gsap.to(chatSupport, { x: 300, duration: 0.3 });
});
...
btnReplenish?.addEventListener('click', () => {
  ...
  // Для реальної інтеграції LiqPay: відправити запит на API
});
